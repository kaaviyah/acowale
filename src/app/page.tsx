  ...
  return (
    <main className="mx-auto w-full max-w-2xl flex-1 px-4 py-8 sm:py-12">
      <header className="mb-6 text-center">
        <div className="inline-block px-3 py-1.5 rounded-full bg-gradient-to-r from-series-1/20 to-series-1/10 border border-series-1/30">
          <p className="text-xs font-black uppercase tracking-widest text-series-1">💬 Acowale Feedback</p>
        </div>
        <h1 className="mt-3 text-4xl sm:text-5xl font-black tracking-tight text-ink whitespace-nowrap">
          We <span className="bg-gradient-to-r from-series-1 to-series-1/60 bg-clip-text text-transparent">value</span> your feedback
        </h1>
        <p className="mt-3 text-base text-ink-secondary leading-relaxed max-w-lg mx-auto">
      ...
      {categories.length > 0 ? (
        ...
        <div className="rounded-2xl border-2 border-series-1/20 bg-gradient-to-br from-series-1/10 to-series-1/5 p-6 text-center shadow-lg shadow-series-1/10">
          ...
          <p className="mt-2 text-sm text-ink-secondary">
            ...
            <code className="font-bold text-series-1 bg-black/10 px-2 py-0.5 rounded-lg inline-block mt-1">pnpm db:seed</code>.
          </p>
        </div>
      )}

      <footer className="mt-8 flex flex-col items-center justify-center border-t-2 border-series-1/10 pt-5 text-sm text-ink-muted gap-2">
        <span className="font-bold">🚀 Acowale CRM · Machine test build</span>
        <Link href="/admin" className="text-series-1 underline underline-offset-2 hover:text-series-1/70 font-bold transition-colors">
          👨‍💼 Team sign in
        </Link>
      </footer>
    </main>
